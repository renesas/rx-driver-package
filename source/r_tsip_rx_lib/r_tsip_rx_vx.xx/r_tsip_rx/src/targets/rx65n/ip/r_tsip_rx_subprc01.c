...
*******************************************************************************************************************/ /**
* @details       RX65NHU TSIP selftest subprocedure 01
* @note          None
*/
void R_TSIP_SelfCheck1SubSub(void)
{
    ...
    RX65NHU_func054(change_endian_long(0x00003008u), change_endian_long(0x00003018u));
    /* WAIT_LOOP */
    while (0u != TSIP.REG_00H.BIT.B31)
    {
        /* waiting */
    }
    RX65NHU_func050(change_endian_long(0x00010001u));
    /* WAIT_LOOP */
    ...
    RX65NHU_func054(change_endian_long(0x10003008u), change_endian_long(0x10003018u));
    /* WAIT_LOOP */
    while (0u != TSIP.REG_00H.BIT.B31)
    {
        /* waiting */
    }
    RX65NHU_func050(change_endian_long(0x00000001u));
    ...
    RX65NHU_func054(change_endian_long(0x72f01007u), change_endian_long(0x72f01017u));
    /* WAIT_LOOP */
    while (0u != TSIP.REG_00H.BIT.B31)
    {
        /* waiting */
    ...
    {
        RX65NHU_func050(change_endian_long(0x00000001u));
        RX65NHU_func054(change_endian_long(0x72f03007u), change_endian_long(0x72f03017u));
        /* WAIT_LOOP */
        while (0u != TSIP.REG_00H.BIT.B31)
        {
            /* waiting */
    ...
    for(iLoop = 0; iLoop < 3; iLoop = iLoop + 1)
    {
        ...
        TSIP.REG_64H.WORD = 0x00000008u;
        TSIP.REG_68H.WORD = 0x00000017u;
        RX65NHU_func054(change_endian_long(0x7af03007u), change_endian_long(0x7af03017u));
        /* WAIT_LOOP */
        while (0u != TSIP.REG_00H.BIT.B31)
        {
            /* waiting */
    ...
    for(iLoop = 0; iLoop < 6; iLoop = iLoop + 1)
    {
        RX65NHU_func052(change_endian_long(0x00810173u));
    }
    for(iLoop = 0; iLoop < 3; iLoop = iLoop + 1)
    {
        RX65NHU_func052(change_endian_long(0x0080197fu));
    }
    for(iLoop = 0; iLoop < 3; iLoop = iLoop + 1)
    {
        RX65NHU_func052(change_endian_long(0x00fe017bu));
    }
    for(iLoop = 0; iLoop < 3; iLoop = iLoop + 1)
    {
        RX65NHU_func052(change_endian_long(0x00af091Fu));
    }
    RX65NHU_func053(change_endian_long(0x000380BBu));
    for(iLoop = 0; iLoop < 6; iLoop = iLoop + 1)
    {
        RX65NHU_func053(change_endian_long(0x000380BBu));
    }
    for(iLoop = 0; iLoop < 3; iLoop = iLoop + 1)
    {
        RX65NHU_func053(change_endian_long(0x000191BBu));
    }
    for(iLoop = 0; iLoop < 3; iLoop = iLoop + 1)
    {
        RX65NHU_func053(change_endian_long(0x000185B7u));
    }
}
/**********************************************************************************************************************
 End of function ./input_dir/RX65NHU/RX65NHU_subprc01_r1.prc
 *********************************************************************************************************************/
TSIP_SEC_DEFAULT
