...
e_tsip_err_t R_TSIP_Aes128CbcEncryptInitPrivate(tsip_aes_key_index_t *key_index, uint32_t *InData_IV)
{
    ...
    && (TSIP_KEY_INDEX_TYPE_TLS_CLIENT_AES128_FOR_CLIENT != key_index->type)
    && (TSIP_KEY_INDEX_TYPE_TLS_SERVER_AES128_FOR_SERVER != key_index->type)
    && (TSIP_KEY_INDEX_TYPE_AES128_FOR_ECDH != key_index->type))
    {
        return TSIP_ERR_KEY_SET;
    }
    else if (TSIP_KEY_INDEX_TYPE_TLS_CLIENT_AES128_FOR_CLIENT == key_index->type)
    {
        indata_cmd = change_endian_long(5); /* CBC-Encrypt for TLS command */
    }
    else if (TSIP_KEY_INDEX_TYPE_TLS_SERVER_AES128_FOR_SERVER == key_index->type)
    {
        indata_cmd = change_endian_long(7); /* CBC-Encrypt for TLS Server command */
    }
    else if (TSIP_KEY_INDEX_TYPE_AES128_FOR_ECDH == key_index->type)
    {
        indata_keytype = change_endian_long(2); /* For ECDH */
...
e_tsip_err_t R_TSIP_Aes128CbcDecryptInitPrivate(tsip_aes_key_index_t *key_index, uint32_t *InData_IV)
{
    ...
    if ((TSIP_KEY_INDEX_TYPE_AES128 != key_index->type)
    && (TSIP_KEY_INDEX_TYPE_TLS_SERVER_AES128_FOR_CLIENT != key_index->type)
    && (TSIP_KEY_INDEX_TYPE_TLS_CLIENT_AES128_FOR_SERVER != key_index->type)
    && (TSIP_KEY_INDEX_TYPE_AES128_FOR_ECDH != key_index->type))
    {
        return TSIP_ERR_KEY_SET;
    }
    else if (TSIP_KEY_INDEX_TYPE_TLS_SERVER_AES128_FOR_CLIENT == key_index->type)
    {
        indata_cmd = change_endian_long(6); /* CBC-Decrypt for TLS command */
    }
    else if (TSIP_KEY_INDEX_TYPE_TLS_CLIENT_AES128_FOR_SERVER == key_index->type)
    {
        indata_cmd = change_endian_long(8); /* CBC-Decrypt for TLS Server command */
    }
    else if (TSIP_KEY_INDEX_TYPE_AES128_FOR_ECDH == key_index->type)
    {
        indata_keytype = change_endian_long(2); /* For ECDH */
...
e_tsip_err_t R_TSIP_Aes256CbcEncryptInitPrivate(tsip_aes_key_index_t *key_index, uint32_t *InData_IV)
{
    ...
    && (TSIP_KEY_INDEX_TYPE_TLS_CLIENT_AES256_FOR_CLIENT != key_index->type)
    && (TSIP_KEY_INDEX_TYPE_TLS_SERVER_AES256_FOR_SERVER != key_index->type)
    && (TSIP_KEY_INDEX_TYPE_AES256_FOR_ECDH != key_index->type))
    {
        return TSIP_ERR_KEY_SET;
    }
    else if (TSIP_KEY_INDEX_TYPE_TLS_CLIENT_AES256_FOR_CLIENT == key_index->type)
    {
        indata_cmd = change_endian_long(5); /* CBC-Encrypt for TLS command */
    }
    else if (TSIP_KEY_INDEX_TYPE_TLS_SERVER_AES256_FOR_SERVER == key_index->type)
    {
        indata_cmd = change_endian_long(7); /* CBC-Encrypt for TLS Server command */
    }
    else if (TSIP_KEY_INDEX_TYPE_AES256_FOR_ECDH == key_index->type)
...
e_tsip_err_t R_TSIP_Aes256CbcDecryptInitPrivate(tsip_aes_key_index_t *key_index, uint32_t *InData_IV)
{
    ...
    if ((TSIP_KEY_INDEX_TYPE_AES256 != key_index->type)
    && (TSIP_KEY_INDEX_TYPE_TLS_SERVER_AES256_FOR_CLIENT != key_index->type)
    && (TSIP_KEY_INDEX_TYPE_TLS_CLIENT_AES256_FOR_SERVER != key_index->type)
    && (TSIP_KEY_INDEX_TYPE_AES256_FOR_ECDH != key_index->type))
    {
        return TSIP_ERR_KEY_SET;
    }
    else if (TSIP_KEY_INDEX_TYPE_TLS_SERVER_AES256_FOR_CLIENT == key_index->type)
    {
        indata_cmd = change_endian_long(6); /* CBC-Decrypt for TLS command */
    }
    else if (TSIP_KEY_INDEX_TYPE_TLS_CLIENT_AES256_FOR_SERVER == key_index->type)
    {
        indata_cmd = change_endian_long(8); /* CBC-Decrypt for TLS Server command */
    }
    else if (TSIP_KEY_INDEX_TYPE_AES256_FOR_ECDH == key_index->type)
        ...
        e_tsip_tls13_mode_t tls13_mode, tsip_tls13_handle_t *tls13_handle, uint32_t *InData_IV)
{
    ...
    if (TSIP_KEY_INDEX_TYPE_TLS_CLIENT_AES128_FOR_CLIENT == key_index->type)
    {
        ...
        indata_datatype = change_endian_long(0); /* For Client */
    }
    else if (TSIP_KEY_INDEX_TYPE_TLS_SERVER_AES128_FOR_SERVER == key_index->type)
    {
        indata_keytype  = change_endian_long(1); /* For TLS */
        indata_datatype = change_endian_long(1); /* For Server */
    }
    else if (TSIP_KEY_INDEX_TYPE_AES128_FOR_ECDH == key_index->type)
    {
        indata_keytype = change_endian_long(2); /* For ECDH */
...
e_tsip_err_t R_TSIP_Sha1HmacGenerateInitPrivate(tsip_hmac_sha_handle_t *handle, tsip_hmac_sha_key_index_t *key_index)
{
    ...
    if (TSIP_KEY_INDEX_TYPE_TLS_CLIENT_HMAC_SHA1_FOR_CLIENT == key_index->type)
    {
        indata_keytype = change_endian_long(1); /* For TLS */
        indata_cmd     = 0; /* For Client */
    }
    else if (TSIP_KEY_INDEX_TYPE_TLS_SERVER_HMAC_SHA1_FOR_SERVER == key_index->type)
    {
        indata_keytype = change_endian_long(1); /* For TLS */
        indata_cmd     = change_endian_long(1); /* For Server */
    }
    else
    {
        indata_keytype = 0; /* For normal */
...
e_tsip_err_t R_TSIP_Sha1HmacVerifyInitPrivate(tsip_hmac_sha_handle_t *handle, tsip_hmac_sha_key_index_t *key_index)
{
    ...
    if (TSIP_KEY_INDEX_TYPE_TLS_SERVER_HMAC_SHA1_FOR_CLIENT == key_index->type)
    {
        indata_keytype = change_endian_long(1); /* For TLS */
        indata_cmd     = 0; /* For Client */
    }
    else if (TSIP_KEY_INDEX_TYPE_TLS_CLIENT_HMAC_SHA1_FOR_SERVER == key_index->type)
    {
        indata_keytype = change_endian_long(1); /* For TLS */
        indata_cmd     = change_endian_long(1); /* For Server */
    }
...
e_tsip_err_t R_TSIP_Sha256HmacGenerateInitPrivate(tsip_hmac_sha_handle_t *handle, tsip_hmac_sha_key_index_t *key_index)
{
    ...
    if (TSIP_KEY_INDEX_TYPE_TLS_CLIENT_HMAC_SHA256_FOR_CLIENT == key_index->type)
    {
        indata_keytype = change_endian_long(1); /* For TLS */
        indata_cmd     = 0; /* For Client */
    }
    else if (TSIP_KEY_INDEX_TYPE_TLS_SERVER_HMAC_SHA256_FOR_SERVER == key_index->type)
    {
        indata_keytype = change_endian_long(1); /* For TLS */
        indata_cmd     = change_endian_long(1); /* For Server */
    }
...
e_tsip_err_t R_TSIP_Sha256HmacVerifyInitPrivate(tsip_hmac_sha_handle_t *handle, tsip_hmac_sha_key_index_t *key_index)
{
    ...
    if (TSIP_KEY_INDEX_TYPE_TLS_SERVER_HMAC_SHA256_FOR_CLIENT == key_index->type)
    {
        indata_keytype = change_endian_long(1); /* For TLS */
        indata_cmd     = 0; /* For Client */
    }
    else if (TSIP_KEY_INDEX_TYPE_TLS_CLIENT_HMAC_SHA256_FOR_SERVER == key_index->type)
    {
        indata_keytype = change_endian_long(1); /* For TLS */
        indata_cmd     = change_endian_long(1); /* For Server */
    }
