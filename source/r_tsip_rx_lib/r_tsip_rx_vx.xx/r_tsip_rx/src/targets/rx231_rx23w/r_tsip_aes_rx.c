 ...
 * Version      : 1.20
 * Description  : Interface definition for the r_tsip_aes_rx TSIP-Lite module.
 *********************************************************************************************************************/
/**********************************************************************************************************************
 ...
 *         : 28.02.2018 1.04     Change API to Init/Update/Final
        ...
        uint32_t ivec_len)
{
    ...
    return R_TSIP_Aes128GcmEncryptInitPrivate(key_index, hashed_ivec);
}
/*******************************
...
* @see           R_TSIP_AesGcmEncryptUpdateTransitionPrivate()
        ...
        uint32_t plain_data_len, uint8_t *aad, uint32_t aad_len)
{
    ...
    if (0 != plain_data_len)
    {
        if (GCM_INPUT_DATA_AAD == handle->flag_update_input_data)
        {
            ...
            R_TSIP_AesGcmEncryptUpdateTransitionPrivate();
        }
        ...
        uint32_t ivec_len)
{
    ...
    handle->flag_update_input_data = GCM_INPUT_DATA_AAD;
    return R_TSIP_Aes128GcmDecryptInitPrivate(key_index, hashed_ivec);
}
/*******************************
...
* @see           R_TSIP_AesGcmDecryptUpdateTransitionPrivate()
        ...
        uint32_t cipher_data_len, uint8_t *aad, uint32_t aad_len)
{
    ...
    if (0 != cipher_data_len)
    {
        if (GCM_INPUT_DATA_AAD == handle->flag_update_input_data)
        {
            ...
            R_TSIP_AesGcmDecryptUpdateTransitionPrivate();
        }
        ...
        uint32_t key_index_word_size, uint32_t *hashed_ivec)
{
    ...
    else
    {
        ...
        {
            ret = R_TSIP_Aes128EncryptDecryptInitSub(&indata_cmd, key_index->value, zero);
            if (TSIP_SUCCESS == ret)
            {
        ...
        {
            ret = R_TSIP_Aes256EncryptDecryptInitSub(&indata_cmd, key_index->value, zero);
            if (TSIP_SUCCESS == ret)
            {
        ...
        uint32_t nonce_len, uint8_t *adata, uint8_t a_len, uint32_t payload_len, uint32_t mac_len)
{
    ...
    return R_TSIP_Aes128CcmEncryptInitPrivate(key_index,
            /* Casting uint32_t pointer is used for address. */
        ...
        uint8_t *mac, uint32_t mac_length)
{
    ...
    ercd = R_TSIP_Aes128CcmEncryptFinalPrivate(&length_tmp,
            /* Casting uint32_t pointer is used for address. */
        ...
        uint32_t nonce_len, uint8_t *adata, uint8_t a_len, uint32_t payload_len, uint32_t mac_len)
{
    ...
    return R_TSIP_Aes128CcmDecryptInitPrivate(key_index,
            /* Casting uint32_t pointer is used for address. */
            (uint32_t*)handle->counter, (uint32_t*)handle->formatted_data, formatted_length);
}
/*******************************
        ...
        uint8_t *mac, uint32_t mac_length)
{
    ...
    ercd = R_TSIP_Aes128CcmDecryptFinalPrivate(
            /* Casting uint32_t pointer is used for address. */
