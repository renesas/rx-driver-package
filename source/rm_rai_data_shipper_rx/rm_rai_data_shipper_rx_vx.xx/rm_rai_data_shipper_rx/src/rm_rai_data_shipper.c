 ...
 **********************************************************************************************************************/
extern fsp_err_t rm_rai_data_shipper_crc_open       (rai_data_shipper_ctrl_t          * const p_api_ctrl);
extern fsp_err_t rm_rai_data_shipper_crc_close      (rai_data_shipper_ctrl_t          * const p_api_ctrl);
extern fsp_err_t rm_rai_data_shipper_crc_calculate  (rai_data_shipper_ctrl_t          * const p_api_ctrl,
                                                     crc_input_t                      * const p_crc_input, 
                                                     uint32_t                         *       calculatedValue);
extern fsp_err_t rm_rai_data_shipper_crc_initialize (rai_data_shipper_ctrl_t          * const p_api_ctrl);
                                    ...
                                    rai_data_shipper_cfg_t const * const p_cfg)
{
    ...
    if (p_cfg->p_crc != NULL)
    {
        err = rm_rai_data_shipper_crc_open(p_ctrl);
                                     ...
                                     rai_data_shipper_write_params_t const * p_write_params)
{
    ...
    if (p_ctrl->p_cfg->p_crc)
    {
...
#if   defined(__CCRX__) || defined(__ICCRX__)   || defined(__RX__)
        rm_rai_data_shipper_crc_initialize(p_ctrl);
#elif defined(__CCRL__) || defined(__ICCRL78__) || defined(__RL78__)
        rm_rai_data_shipper_crc_initialize(p_ctrl);
#else
#endif
        ...
        input.crc_seed       = crc;
        rm_rai_data_shipper_crc_calculate(p_ctrl, &input, &crc);
...
fsp_err_t RM_RAI_DATA_SHIPPER_Close (rai_data_shipper_ctrl_t * const p_api_ctrl)
{
    ...
    if (p_ctrl->p_cfg->p_crc)
    {
        rm_rai_data_shipper_crc_close(p_ctrl);
    }
