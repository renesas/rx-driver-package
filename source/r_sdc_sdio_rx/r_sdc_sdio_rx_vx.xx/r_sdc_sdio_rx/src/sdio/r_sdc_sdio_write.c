...
*              :    int32_t     cnt                     ;   Number of write (byte)
...
sdc_sd_status_t r_sdc_sdio_Write(uint32_t card_no, sdc_sdio_access_t * p_sdc_sdio_access)
{
    ...
    {
        SDC_SD_LOG_FUNC(SDC_SD_DEBUG_ERR_ID, (uint32_t)SDC_SDIO_WRITE, __LINE__);
        return SDC_SD_ERR;
    }
    ...
    /* ==== Execute I/O Write Extended operation. ---- */
    if (R_SDHI_SetClock(p_hndl->channel, p_hndl->io_tran_speed, SDHI_CLOCK_ENABLE) == SDHI_SUCCESS)
    {
        ...
        /* ---- Executes I/O Write Extended operation. ---- */
        if ((p_sdc_sdio_access->op_code & SDC_SDIO_BLOCK_MODE))
        {
            R_BSP_NOP();    /* Because a compiler doesn't put an assembler out. */
            r_sdc_sdio_write_block_sub(card_no, p_sdc_sdio_access);
        }
        else
        {
            R_BSP_NOP();    /* Because a compiler doesn't put an assembler out. */
            r_sdc_sdio_write_byte_sub(card_no, p_sdc_sdio_access);
        }
    }
    else
    {
...
/**********************************************************************************************************************
* Outline      : I/O Write using Block Mode and Byte Mode
* Function Name: r_sdc_sdio_write_block_sub
* Description  : Writes data of the multiple block length using CMD53 Block mode and 
*              : writes the rest data using CMD53 Byte mode.
* Arguments    : uint32_t       card_no                 ;   SD Card No.
*              : sdc_sdio_access_t * p_sdc_sdio_access
*              :    uint8_t     * p_buff                ;   Data buffer
*              :    uint32_t    func                    ;   Function number
...
sdc_sd_status_t r_sdc_sdio_write_block_sub(uint32_t card_no, sdc_sdio_access_t * p_sdc_sdio_access)
{
    ...
    {
        /* ==== Multiple Block Write by Blocklen size ==== */
        ...
        for (i = block_cnt; i > 0; i = (i - SDC_SDIO_TRANS_SECTORS))
        {
            ...
            arg = ((p_sdc_sdio_access->func << SDC_SD_BIT_SHIFT_28)
                 | SDC_SDIO_BLOCKMODE_RWFLAG_W
                 | (op_code_addr << SDC_SD_BIT_SHIFT_26)
                 | (p_sdc_sdio_access->adr << SDC_SD_BIT_SHIFT_9)
                 | (uint32_t)block_cnt);

            /* Issue CMD53 Block Mode. */
            if (r_sdc_sd_send_cmd_arg(card_no, SDC_SD_CMD53_W_BLOCK, SDC_SD_RESP_R5, arg) != SDC_SD_SUCCESS)
            {
                SDC_SD_LOG_FUNC(SDC_SD_DEBUG_ERR_ID, (uint32_t)SDC_SDIO_WRITE, __LINE__);
                r_sdc_sdio_read_write_error(card_no, p_sdc_sdio_access->func);
                return p_hndl->error;
            ...
            if (SDC_SD_MODE_SW == p_sdc_sdio_access->trans_mode)
            {
                ...
                /* Software data transfer
                   BWE bit is cleared in the following function. */
            ...
            ret = r_sdc_sd_int_io_wait(card_no, SDC_SD_TIMEOUT_RESP, p_sdc_sdio_access->func);

            if ((SDC_SD_MODE_DMA | SDC_SD_MODE_DTC) & p_sdc_sdio_access->trans_mode)
            {
                r_sdc_sd_disable_icu_int(card_no);
            ...
            /* Clear All end bit. 
               This is no problem. Surely, clear BWE and errors, too. */
/*          r_sdc_sd_clear_sdsts(card_no, SDHI_SDIMSK1_DATA_TRNS, 0); */

            /* Clear All end, BWE and errors. 
               SDHI_SDSTS1 and SDHI_SDSTS2 are already cleared in interrupt handler.*/
/*          R_SDHI_ClearSdstsReg(p_hndl->channel, SDHI_SDIMSK1_TRNS_RESP, SDHI_SDIMSK2_BWE); */
            r_sdc_sd_clear_sdsts(card_no, SDHI_SDIMSK1_DATA_TRNS, SDHI_SDIMSK2_BWE);
    ...
    {
        /* ==== Multiple Byte Write ==== */
        /* ---- Executes write operation using CMD53 Byte mode. ---- */
        ...
        op_code_addr = (p_sdc_sdio_access->op_code & SDC_SDIO_INCREMENT_ADDR);
        arg = ((p_sdc_sdio_access->func << SDC_SD_BIT_SHIFT_28)
             | SDC_SDIO_BYTEMODE_RWFLAG_W
             | (op_code_addr << SDC_SD_BIT_SHIFT_26)
             | (p_sdc_sdio_access->adr << SDC_SD_BIT_SHIFT_9)
             | (uint32_t)rem);

        /* Issue CMD53 Byte mode. */
        if (r_sdc_sd_send_cmd_arg(card_no, SDC_SD_CMD53_W_BYTE, SDC_SD_RESP_R5, arg) != SDC_SD_SUCCESS)
        {
            SDC_SD_LOG_FUNC(SDC_SD_DEBUG_ERR_ID, (uint32_t)SDC_SDIO_WRITE, __LINE__);
            r_sdc_sdio_read_write_error(card_no, p_sdc_sdio_access->func);
            return p_hndl->error;
        ...
        /* Software data transfer
           BWE bit is cleared in the following function. */
        trans_ret = r_sdc_sd_software_trans_blk(card_no, p_sdc_sdio_access->p_buff, 1, SDC_SD_TRANS_WRITE, rem,
                                              p_sdc_sdio_access->func);
...
} /* End of function r_sdc_sdio_write_block_sub() */

/**********************************************************************************************************************
* Outline      : I/O Write using Byte Mode Only
* Function Name: r_sdc_sdio_write_byte_sub
* Description  : Writes data using CMD53 Byte mode only.
* Arguments    : uint32_t       card_no                 ;   SD Card No.
*              : sdc_sdio_access_t * p_sdc_sdio_access
*              :    uint8_t     * p_buff                ;   Data buffer
*              :    uint32_t    func                    ;   Function number
...
sdc_sd_status_t r_sdc_sdio_write_byte_sub(uint32_t card_no, sdc_sdio_access_t * p_sdc_sdio_access)
{
    ...
    {
        /* ==== Multiple Byte Write by I/O Block Size ==== */
        /* ---- Executes write operation using CMD53 Byte mode. ---- */
...
} /* End of function r_sdc_sdio_write_byte_sub() */
