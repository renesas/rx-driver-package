 ...
 * If using RX FIT module,
 * In Light/Proximity/Light Proximity mode, if device interrupt is enabled, blocks 2 bytes on the I2C bus.
 * In PPG mode, if device interrupt is enabled, blocks 6 bytes on the I2C bus. If device interrupt is disabled,
 ...
 * If using RX FIT module,
 * In Light mode, if device interrupt is enabled, blocks 2 bytes on the I2C bus.
 ...
 * If using RX FIT module,
 * In Proximity mode, if device interrupt is enabled, blocks 2 bytes on the I2C bus.
 ...
 * If using RX FIT module,
 * In Light Proximity mode, blocks 2 bytes on the I2C bus.
...
#elif (BSP_CFG_RTOS == 0) && (defined(__CCRX__) || defined(__ICCRX__) || defined(__RX__))
                /* Set flag */
                p_ctrl->rx_communication_finished = true;
#else
