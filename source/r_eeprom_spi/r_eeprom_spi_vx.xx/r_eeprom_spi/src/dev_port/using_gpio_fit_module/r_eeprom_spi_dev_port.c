...
void r_eeprom_spi_cs_init(uint8_t devno)
{
    ...
    switch (devno)
    {
        case EEPROM_SPI_DEV0:
            ...
            R_GPIO_PinControl(EEPROM_SPI_DEV0_GPIO_CS, GPIO_CMD_ASSIGN_TO_GPIO);
                                                            /* Use as a general I/O port.       */
            config.pin_function  = 0x00;
            config.irq_enable    = 0;
            config.analog_enable = 0;
            R_MPC_Write(EEPROM_SPI_DEV0_GPIO_CS, &config);  /* Pin Function Select : Hi-Z       */
            R_GPIO_PinWrite(EEPROM_SPI_DEV0_GPIO_CS, GPIO_LEVEL_HIGH);
                                                            /* Output data is High.             */
            R_GPIO_PinDirectionSet(EEPROM_SPI_DEV0_GPIO_CS, GPIO_DIRECTION_OUTPUT);
        ...
        case EEPROM_SPI_DEV1:
            ...
            R_GPIO_PinControl(EEPROM_SPI_DEV1_GPIO_CS, GPIO_CMD_ASSIGN_TO_GPIO);
                                                            /* Use as a general I/O port.       */
            config.pin_function  = 0x00;
            config.irq_enable    = 0;
            config.analog_enable = 0;
            R_MPC_Write(EEPROM_SPI_DEV1_GPIO_CS, &config);  /* Pin Function Select : Hi-Z       */
            R_GPIO_PinWrite(EEPROM_SPI_DEV1_GPIO_CS, GPIO_LEVEL_HIGH);
                                                            /* Output data is High.             */
            R_GPIO_PinDirectionSet(EEPROM_SPI_DEV1_GPIO_CS, GPIO_DIRECTION_OUTPUT);
                                                            /* Select output direction.         */
...
void r_eeprom_spi_set_cs(uint8_t devno, uint8_t lv)
{
    ...
    switch (devno)
    {
        case EEPROM_SPI_DEV0:
#if defined(EEPROM_SPI_CFG_DEV0_INCLUDED)
            R_GPIO_PinWrite(EEPROM_SPI_DEV0_GPIO_CS, level);
#endif  /* #if defined(EEPROM_SPI_CFG_DEV0_INCLUDED) */
        break;
        case EEPROM_SPI_DEV1:
#if defined(EEPROM_SPI_CFG_DEV1_INCLUDED)
            R_GPIO_PinWrite(EEPROM_SPI_DEV1_GPIO_CS, level);
#endif  /* #if defined(EEPROM_SPI_CFG_DEV1_INCLUDED) */
        break;
        default:
            /* Do nothing. */
