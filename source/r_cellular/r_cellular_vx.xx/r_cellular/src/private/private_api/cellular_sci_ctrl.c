 ...
 *********************************************************************************************************************/
static volatile st_cellular_ctrl_t * sp_cellular_ctrl;
...
e_cellular_err_t cellular_serial_open(st_cellular_ctrl_t * const p_ctrl)
{
    ...
    uint8_t             priority = CELLULAR_CFG_INT_PRIORITY - 1;
#endif

    sp_cellular_ctrl = p_ctrl;
    ...
    sci_cfg.async.int_priority      = CELLULAR_CFG_INT_PRIORITY;    // 1=lowest, 15=highest
...
static void cellular_uart_callback(void * const p_Args)
{
    ...
    switch (p_args->event)
    {
        ...
        case SCI_EVT_TEI:
        {
            sp_cellular_ctrl->sci_ctrl.tx_end_flg = CELLULAR_TX_END_FLAG_ON;
            break;
        }
#endif
        case SCI_EVT_RXBUF_OVFL:
        {
            sp_cellular_ctrl->sci_ctrl.sci_err_flg = SCI_EVT_RXBUF_OVFL;
            break;
        }
        case SCI_EVT_OVFL_ERR:
        {
            sp_cellular_ctrl->sci_ctrl.sci_err_flg = SCI_EVT_OVFL_ERR;
            break;
        }
        case SCI_EVT_FRAMING_ERR:
        {
            sp_cellular_ctrl->sci_ctrl.sci_err_flg = SCI_EVT_FRAMING_ERR;
            break;
        }
        case SCI_EVT_PARITY_ERR:
        {
            sp_cellular_ctrl->sci_ctrl.sci_err_flg = SCI_EVT_PARITY_ERR;
            break;
        }
        default:
        {
