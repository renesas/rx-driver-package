...
e_cellular_err_t atc_cgmr(st_cellular_ctrl_t * const p_ctrl)
{
    ...
    atc_generate(p_ctrl->sci_ctrl.atc_buff,
        (const uint8_t *)&gp_at_command[ATC_GET_SW_VERSION][0],   // (const uint8_t *const *)->(const uint8_t **)
            NULL);

    at_ret = cellular_execute_at_command(p_ctrl, p_ctrl->sci_ctrl.atc_timeout, ATC_RETURN_OK, ATC_GET_SW_VERSION);
