...
typedef enum e_ri3c_type
{
    ...
    /** A target device listens to the bus for relevant I3C Commands (CCCs) sent by the current controller, and responds accordingly.
     * Target devices may also initiate In-band interrupts and Hot-Join requests. */
...
{
    RI3C_DEVICE_PROTOCOL_I2C,           ///< Transfers will use legacy I2C protocal with open-drain output at a reduced baudrate.
    RI3C_DEVICE_PROTOCOL_I3C,           ///< Transfers will use I3C SDR mode.
...
{
    RI3C_IBI_TYPE_INTERRUPT,                ///< Application specific In-Band Interrupt for notifying the controller when an event occurs.
    RI3C_IBI_TYPE_HOT_JOIN,                 ///< Request the controller to perform the Dynamic Address Assignment process.
    RI3C_IBI_TYPE_CONTROLLERROLE_REQUEST    ///< Request the controller to give up control of the bus.
} ri3c_ibi_type_t;
...
typedef struct s_ri3c_device_table_cfg
{
    ...
    ri3c_device_protocol_t device_protocol;     ///< The protocol used to communicate with this device (I3C / I2C Legacy).
    bool ibi_accept;                            ///< Accept or reject IBI requests from this device.
    bool controllerrole_request_accept;         ///< Accept controller role requests from this device.
...
{
    uint8_t static_address;            ///< I3C Static address.

    /** Dynamic address for this device. Note that the dynamic address will automatically be updated when ENTDAA is
     * completed.*/
    uint8_t dynamic_address;

    ri3c_target_info_t target_info;       ///< PID, BCR, and DCR registers for the device (Target mode only).
} ri3c_device_cfg_t;
...
{
    ri3c_event_t               event;             ///< The type of event that has occurred.
    uint32_t                   event_status;      ///< Status flags associated with the event.
    uint32_t                   transfer_size;     ///< Number of bytes transferred.
    ri3c_target_info_t const * p_target_info;     ///< A pointer to the Characteristics Registers read during ENTDAA.
    uint8_t                    dynamic_address;   ///< The dynamic address that was assigned to the target during ENTDAA.
    ri3c_ibi_type_t            ibi_type;          ///< The type of IBI that has been received.
...
typedef struct st_ri3c_api
{
     ...
     * when it is selected by deviceSelect.
     *
    ...
    /** Start Dynamic Address Assignment by sending either the ENTDAA or SETDASA command See ri3c_address_assignment_mode_t
     ...
     * @param[in]   device_count            The number of devices to assign (Only used with RI3C_ADDRESS_ASSIGNMENT_MODE_ENTDAA).
     */
    ...
    /**
     * In controller mode: Start a write transfer. When the transfer is completed send a stop condition or a repeated-start.
     * In target mode:  Set the write buffer and configure the number of bytes that will be transferred before the the transfer
     *                 is ended by the target via the 'T' bit or by the controller issueing a stop condition.
     *
     * @param[in]   p_ctrl     Control block set this instance.
    ...
    /**
     * In controller mode: Start a read transfer. When the transfer is completed, send a stop condition or a repeated-start.
     * In target mode: Set the read buffer for storing data read during the transfer. When the buffer is full, the application
