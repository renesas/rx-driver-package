...
typedef enum _flash_block_address
{
...
#if   MCU_CFG_PART_MEMORY_SIZE == 0xB   /*   'B' parts 256KB ROM */
    FLASH_CF_BLOCK_INVALID = 0xFFFBFFFF,
#else
...
#if   MCU_CFG_PART_MEMORY_SIZE == 0xC   /*   'C' parts 384KB ROM */
    FLASH_CF_BLOCK_INVALID = 0xFFF9FFFF,
#else
    ...
    FLASH_CF_BLOCK_INVALID = 0xFFF7FFFF,
#endif
#endif
...
/*  According to HW Manual Table 37.42 the Max Programming Time for 8 bytes (ROM)
...
/*  According to HW Manual Table 37.45 the Max Programming Time for 1 bytes
...
/*  According to HW Manual Table 37.45 the Max Blank Check time for 1 bytes
...
/*  According to HW Manual Table 37.42 the Max Erasure Time for a 2KB block is
...
/*  According to HW Manual Table 37.45 the Max Erasure Time for a 1KB Data Flash block is
    around 496.5 ms.  This is with a FCLK of 1MHz. The calculation below
    calculates the number of ICLK ticks needed for the timeout delay.
    The 498.5 ms number is adjusted linearly depending on the FCLK frequency.
*/
