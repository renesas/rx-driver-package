 ...
 * @defgroup RM_COMMS_UART_API Communicatons Middleware Interface
 * @brief Interface for Communications Middleware functions.
 *
 * @section RM_COMMS_UART_API_Summary Summary
...
#ifndef RM_COMMS_UART_API_H_
#define RM_COMMS_UART_API_H_
...
typedef enum e_rm_comms_uart_event
{
    ...
    RM_COMMS_UART_EVENT_TX_OPERATION_COMPLETE,
    RM_COMMS_UART_EVENT_RX_OPERATION_COMPLETE,
    RM_COMMS_UART_EVENT_ERROR,
} rm_comms_uart_event_t;

/** Struct to pack params for writeRead */
typedef struct st_rm_comms_uart_write_read_params
{
    uint8_t * p_src;
    uint8_t * p_dest;
    uint8_t   src_bytes;
...
typedef struct st_rm_comms_uart_cfg
{
    ...
    void (* p_callback)(rm_comms_uart_callback_args_t * p_args); ///< Pointer to callback function, mostly used if using non-blocking functionality.
} rm_comms_uart_cfg_t;

/** Communications control block.  Allocate an instance specific control block to pass into the Communications API calls.
 * @par Implemented as
 * - rm_comms_uart_i2c_instance_ctrl_t
 */
typedef void rm_comms_uart_ctrl_t;

/** COMM APIs */
typedef struct st_rm_comms_uart_api
{
    /** Open driver.
     * @par Implemented as
     ...
     */
    fsp_err_t (* open)(rm_comms_uart_ctrl_t * const p_ctrl, rm_comms_uart_cfg_t const * const p_cfg);
     ...
     * @param[in]  p_ctrl       Pointer to control structure.
     */
    fsp_err_t (* close)(rm_comms_uart_ctrl_t * const p_ctrl);
     ...
     */
    fsp_err_t (* read)(rm_comms_uart_ctrl_t * const p_ctrl, uint8_t * const p_dest, uint32_t const bytes);
     ...
     */
    fsp_err_t (* write)(rm_comms_uart_ctrl_t * const p_ctrl, uint8_t * const p_src, uint32_t const bytes);
     ...
     */
    fsp_err_t (* writeRead)(rm_comms_uart_ctrl_t * const p_ctrl, rm_comms_uart_write_read_params_t write_read_params);
     ...
     */
    fsp_err_t (* callbackSet)(rm_comms_uart_ctrl_t * const p_api_ctrl, void (* p_callback)(rm_comms_uart_callback_args_t *), void const * const p_context);
} rm_comms_uart_api_t;

/** This structure encompasses everything that is needed to use an instance of this interface. */
typedef struct st_rm_comms_uart_instance
{
    rm_comms_uart_ctrl_t      * p_ctrl;
    rm_comms_uart_cfg_t const * p_cfg;
    rm_comms_uart_api_t const * p_api;
} rm_comms_uart_instance_t;
...
#endif                                 /* RM_COMMS_UART_API_H_ */

/*******************************************************************************************************************//**
 * @} (end addtogroup RM_COMMS_UART_API)
 **********************************************************************************************************************/
