...
static uint32_t lpt_regval_calc(uint32_t const period, uint32_t const srcclk, uint8_t const divratio)
{
    ...
    calc_work = (uint64_t)srcclk * (uint64_t)period; /* It's multiplication of 32 bits and 32 bits, so 64 bits aren't exceeded. */
    calc_work *= (uint64_t)LPT_CALCSCALE_OFFSET;     /* It'll be an appropriate area because 32 bits are being expanded in 64 bits. */
    calc_work /= (uint64_t)divratio;                 /* It'll be an appropriate area because 8 bits are being expanded in 64 bits. */
    calc_work += (uint64_t)((LPT_CONV_US * LPT_CALCSCALE_OFFSET) / 2);       /* add denominator/2 for rounding */

    retval = (uint32_t)(calc_work / (uint64_t)(LPT_CONV_US * LPT_CALCSCALE_OFFSET)) -1; /* It'll be an appropriate area because 
	                                                                                       32 bits are being expanded in 64 bits. */
    return retval;
}
/*****************************************************************************
