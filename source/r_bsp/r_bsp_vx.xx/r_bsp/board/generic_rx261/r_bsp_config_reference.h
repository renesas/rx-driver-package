...
*         : 27.11.2024 1.01      Changed comment of BSP_CFG_RTC_ENABLE.
*         : 26.02.2025 1.02      Changed the disclaimer.
*         : 28.05.2025 1.03      Added the following macro definitions.
...
/* NOTE:
   The default settings are the same as when using EK-RX261.
...
/* Enter the product part number for your MCU. This information will be used to obtain information about your MCU such 
   ...
   | | | |  |  | |______BSP_CFG_MCU_PART_VERSION             = Chip version.
...
/* Chip version.
   ...
   A            = 0xA             = Chip version A
                                  = Encryption module not included, USB included, Only CAN 2.0 protocol supported
   B            = 0xB             = Chip version B
                                  = Encryption module and USB included, CAN FD protocol supported
*/
#define BSP_CFG_MCU_PART_VERSION        (0xA)

/* ROM, RAM, and Data Flash Capacity.
   Character(s) = Value for macro = ROM Size/Ram Size/Data Flash Size
   8            = 0x8             = 512KB/128KB/8KB
   7            = 0x7             = 384KB/128KB/8KB
   6            = 0x6             = 256KB/128KB/8KB
*/
...
/* Group name.
   ...
   61           = RX261 Group
*/
#define BSP_CFG_MCU_PART_GROUP          "RX261"
...
/* USB Clock source select (USBCKSEL). Choose which clock source to input to the USB circuit.
   Available clock sources:
   0 = HOCO
   2 = LOCO
   5 = PLL Circuit
   6 = PLL 2 Circuit (default)
*/
#define BSP_CFG_USB_CLOCK_SOURCE        (6)

/* CANFD Clock source select (CANFDCKSEL). Choose which clock source to input to the CANFD circuit.
   Available clock sources:
   2 = LOCO
   5 = PLL Circuit (default)
   6 = PLL 2 Circuit
*/
#define BSP_CFG_CANFD_CLOCK_SOURCE      (5)
...
/* Sub clock Oscillator Switching (SOSEL).
   0 = Resonator
   1 = External clock input
*/
#define BSP_CFG_SUB_CLOCK_SOURCE        (0)
...
/* USB Clock supply control (USBCKSREQ).
   0 = Stop clock supply to USB.
   1 = Supply clock to USB. (default)
*/
#define BSP_CFG_USB_CLOCK_ENABLE        (1)

/* CANFD Clock supply control (CANFDCKSREQ).
   0 = Stop clock supply to CANFD.
   1 = Supply clock to CANFD. (default)
*/
#define BSP_CFG_CANFD_CLOCK_ENABLE      (1)
...
/* Clock configuration options.
   ...
   ICLK is 64MHz, PCLKA is 64MHz, PCLKB is 32MHz, PCLKD is 64MHz, FCLK is 64MHz, USB Clock is 48MHz,
   CANFD Clock is 60MHz,
   and then the settings would be:

   BSP_CFG_XTAL_HZ  = 8000000
   BSP_CFG_PLL_DIV  = 1        (no division)
   BSP_CFG_PLL_MUL  = 8.0      (8MHz x 8.0 = 64MHz)
   BSP_CFG_PLL2_DIV = 1        (no division)
   BSP_CFG_PLL2_MUL = 6.0      (8MHz x 6.0 = 48MHz)
   BSP_CFG_ICK_DIV  = 1      : System Clock (ICLK)        = 
                               (((BSP_CFG_XTAL_HZ/BSP_CFG_PLL_DIV) * BSP_CFG_PLL_MUL) / BSP_CFG_ICK_DIV)   = 64MHz
   BSP_CFG_PCKA_DIV = 1      : Peripheral Clock A (PCLKA) = 
                               (((BSP_CFG_XTAL_HZ/BSP_CFG_PLL_DIV) * BSP_CFG_PLL_MUL) / BSP_CFG_PCKA_DIV)  = 64MHz
   ...
   BSP_CFG_PCKD_DIV = 1      : Peripheral Clock D (PCLKD) = 
                               (((BSP_CFG_XTAL_HZ/BSP_CFG_PLL_DIV) * BSP_CFG_PLL_MUL) / BSP_CFG_PCKD_DIV)  = 64MHz
   BSP_CFG_FCK_DIV  = 1      : Flash IF Clock (FCLK)      = 
                               (((BSP_CFG_XTAL_HZ/BSP_CFG_PLL_DIV) * BSP_CFG_PLL_MUL) / BSP_CFG_FCK_DIV)   = 64MHz
   BSP_CFG_CANFD_CLOCK_SOURCE = 5 : CANFD Clock (CANFDCLK) = 
   BSP_CFG_CFDCK_DIV = 2          : (((BSP_CFG_XTAL_HZ/BSP_CFG_PLL_DIV) * BSP_CFG_PLL_MUL) / BSP_CFG_CFDCK_DIV) = 32MHz
   BSP_CFG_USB_CLOCK_SOURCE = 6   : USB Clock (UCLK)       = 
                                    (((BSP_CFG_XTAL_HZ/BSP_CFG_PLL2_DIV) * BSP_CFG_PLL2_MUL)               = 48MHz
*/
...
/* PLL Frequency Multiplication Factor Select (STC).
   Available multipliers = x4.0 to x15.5 in 0.5 increments (e.g. 4.0, 4.5, 5.0, ..., 14.5, 15.0, 15.5)
*/
#define BSP_CFG_PLL_MUL                 (8.0)

/* PLL2 Input Frequency Division Ratio Select (PLIDIV).
   Available divisors = /1 (no division), /2, /4
*/
#define BSP_CFG_PLL2_DIV                (1)

/* PLL2 Frequency Multiplication Factor Select (STC).
   Available multipliers = x4.0 to x15.5 in 0.5 increments (e.g. 4.0, 4.5, 5.0, ..., 14.5, 15.0, 15.5)
*/
#define BSP_CFG_PLL2_MUL                (6.0)
...
/* CANFD Clock Divider Select (CANFDCKDIV).
   Available divisors = /1 (no division), /2, /4, /8
*/
#define BSP_CFG_CFDCK_DIV               (2)
...
/* Configure whether voltage detection 0 circuit and HOCO are enabled after reset.
   OFS1 - Option Function Select Register 1
       ...
       b7:b5   Reserved (set to 1)
       b4      VDSEL2 - Voltage Detection 0 Level Select 2
             0 : 1.69 V
             1 : Voltage selected by VDSEL bit
       ...
       b1:b0   VDSEL - Voltage Detection 0 Level Select
             ...
             1 0: 2.53 V
             1 1: 1.90 V
   Default value is 0xFFFFCFFF.
*/
#define BSP_CFG_OFS1_REG_VALUE  (0xFFFFCFFF)
...
*/
#define BSP_CFG_CONFIGURATOR_VERSION                (2180)
