...
/* This macro runs or stops the PLL circuit.
   ...
   2. Supply clock to USB and USB clock source is PLL circuit.
   3. Supply clock to CANFD and CANFD clock source is PLL circuit.
   4. Clock output enable and clock output source is PLL circuit.
 */
#if (BSP_CFG_CLOCK_SOURCE == 4) \
    || ((BSP_CFG_USB_CLOCK_ENABLE == 1) && (BSP_CFG_USB_CLOCK_SOURCE == 5)) \
    || ((BSP_CFG_CANFD_CLOCK_ENABLE == 1) && (BSP_CFG_CANFD_CLOCK_SOURCE == 5)) \
...
/* This macro runs or stops the PLL2 circuit.
   If the following conditions are satisfied, PLL2 circuit will operate.
   1. Supply clock to USB and USB clock source is PLL2 circuit.
   2. Supply clock to CANFD and CANFD clock source is PLL2 circuit.
 */
#if ((BSP_CFG_USB_CLOCK_ENABLE == 1) && (BSP_CFG_USB_CLOCK_SOURCE == 6)) \
    || ((BSP_CFG_CANFD_CLOCK_ENABLE == 1) && (BSP_CFG_CANFD_CLOCK_SOURCE == 6))
    #define BSP_PRV_PLL2_CLK_OPERATING   (1)    /* PLL2 circuit is operating. */
#else /* PLL is not used as clock source. */
    #define BSP_PRV_PLL2_CLK_OPERATING   (0)    /* PLL2 circuit is stopped. */
#endif

#if BSP_CFG_BOOTLOADER_PROJECT == 1
/* Enable the following macro definitions in the bootloader project. */
#define BSP_PRV_SCKCR_RESET_VALUE        (0x33003303)
#define BSP_PRV_SCKCR3_RESET_VALUE       (0x0000)
#define BSP_PRV_MEMWAITR_RESET_VALUE     (0x0000)
#define BSP_PRV_MEMWAITR_RESETTING_VALUE (0xAA00)
...
static void bsp_usb_clock_initial_configure (void);
static void bsp_canfd_clock_initial_configure (void);
...
static void operating_frequency_set(void)
{
    ...
    /* b23 to b16 and b7 to b4 should be 0x0. */
...
#if BSP_CFG_BOOTLOADER_PROJECT == 0
/* Disable the following functions in the bootloader project. */
    /* USB clock initial setting. */
    bsp_usb_clock_initial_configure();

    /* CANFD clock initial setting. */
    bsp_canfd_clock_initial_configure();
...
#else /* BSP_CFG_MAIN_CLOCK_OSCILLATE_ENABLE == 1 */
...
/* Disable the following functions in the bootloader project. */
    ...
    if (0 == SYSTEM.RSTSR1.BIT.CWSF)
    {
        ...
        /* Set the oscillation source of the sub clock oscillator. */
        SYSTEM.SOMCR.BIT.SOSEL = BSP_CFG_SUB_CLOCK_SOURCE;

        /* Set the Sub-Clock Oscillator Drive Capacity Control. */
        SYSTEM.SOMCR.BIT.SODRV = tmp_sodrv;

        /* WAIT_LOOP */
        while (tmp_sodrv != SYSTEM.SOMCR.BIT.SODRV)
        {
...
#if BSP_CFG_BOOTLOADER_PROJECT == 0
/* Disable the following functions in the bootloader project. */
#if BSP_PRV_PLL2_CLK_OPERATING == 1
    /* Set PLL Input Divisor. */
    SYSTEM.PLL2CR.BIT.PLIDIV = BSP_CFG_PLL2_DIV >> 1;

    /* Set PLL Multiplier. */
    SYSTEM.PLL2CR.BIT.STC = ((uint8_t)((float)BSP_CFG_PLL2_MUL * 2.0)) - 1;

    /* Set the PLL to operating. */
    SYSTEM.PLL2CR2.BYTE = 0x00;

    /* WAIT_LOOP */
    while (0 == SYSTEM.OSCOVFSR.BIT.PL2OVF)
    {
        /* The delay period needed is to make sure that the PLL has stabilized.
           If you use simulator, the flag is not set to 1, resulting in an infinite loop. */
        R_BSP_NOP();
    ...
    /* Set to memory wait and data flash wait if ICLK is > 32MHz. */
    if (BSP_ICLK_HZ > BSP_MEMORY_NO_WAIT_MAX_FREQUENCY)
    {
        /* Set memory wait cycle. */
    ...
    /* Disable the following functions in the bootloader project. */
/***********************************************************************************************************************
* Function name: bsp_usb_clock_initial_configure
* Description  : Configures the USB clock initial settings
* Arguments    : none
* Return value : none
***********************************************************************************************************************/
static void bsp_usb_clock_initial_configure(void)
{
#if BSP_CFG_USB_CLOCK_ENABLE == 1
    #if BSP_CFG_USB_CLOCK_SOURCE == 2
    /* LOCO is chosen. This is the default out of reset. */
    #else
    /* Other than LOCO. */

    /* Stop clock supply to USB. */
    SYSTEM.USBCKCR.BIT.USBCKSREQ = 1;

    /* WAIT_LOOP */
    while (0 == SYSTEM.USBCKCR.BIT.USBCKSRDY)
    {
        /* do nothing. Wait until clock supply stops. */
    }

    /* Switch the USB clock source. */
    SYSTEM.USBCKCR.BIT.USBCKSEL = BSP_CFG_USB_CLOCK_SOURCE;

    /* Start clock supply to USB. */
    SYSTEM.USBCKCR.BIT.USBCKSREQ = 0;

    /* WAIT_LOOP */
    while (1 == SYSTEM.USBCKCR.BIT.USBCKSRDY)
    {
        /* do nothing. Wait until clock supply starts. */
    }
    #endif /* BSP_CFG_USB_CLOCK_SOURCE == 1 */
#else /* BSP_CFG_USB_CLOCK_ENABLE == 0 */
    /* Stop clock supply to USB. */
    SYSTEM.USBCKCR.BIT.USBCKSREQ = 1;
...
#endif /* BSP_CFG_USB_CLOCK_ENABLE == 1 */
} /* End of function bsp_usb_clock_initial_configure() */

/***********************************************************************************************************************
* Function name: bsp_canfd_clock_initial_configure
* Description  : Configures the CANFD clock initial settings
* Arguments    : none
* Return value : none
***********************************************************************************************************************/
static void bsp_canfd_clock_initial_configure(void)
{
#if BSP_CFG_CANFD_CLOCK_ENABLE == 1
    #if BSP_CFG_CANFD_CLOCK_SOURCE == 0
    /* LOCO is chosen. This is the default out of reset. */
    #else
    /* Other than LOCO. */

    /* Stop clock supply to CANFD. */
    SYSTEM.CANFDCKCR.BIT.CANFDCKSREQ = 1;

    /* WAIT_LOOP */
    while (0 == SYSTEM.CANFDCKCR.BIT.CANFDCKSRDY)
    {
        /* do nothing. Wait until clock supply stops. */
    }

    /* Set CANFD Divisor. */
    SYSTEM.CANFDCKDIVCR.BIT.CANFDCKDIV = BSP_CFG_CFDCK_DIV >> 1;

    /* Switch the CANFD clock source. */
    SYSTEM.CANFDCKCR.BIT.CANFDCKSEL = BSP_CFG_CANFD_CLOCK_SOURCE;

    /* Start clock supply to CANFD. */
    SYSTEM.CANFDCKCR.BIT.CANFDCKSREQ = 0;

    /* WAIT_LOOP */
    while (1 == SYSTEM.CANFDCKCR.BIT.CANFDCKSRDY)
    {
        /* do nothing. Wait until clock supply starts. */
    }
    #endif /* BSP_CFG_CANFD_CLOCK_SOURCE == 1 */
#else /* BSP_CFG_CANFD_CLOCK_ENABLE == 0 */
    /* Stop clock supply to CANFD. */
    SYSTEM.CANFDCKCR.BIT.CANFDCKSREQ = 1;
...
#endif /* BSP_CFG_CANFD_CLOCK_ENABLE == 1 */
} /* End of function bsp_canfd_clock_initial_configure() */
...
void bsp_mcu_clock_reset_bootloader(void)
{
    ...
    if(BSP_PRV_MEMWAITR_RESET_VALUE != FLASH.MEMWAITR.WORD)
    {
        /* Reset Memory Wait Cycle */
        FLASH.MEMWAITR.WORD = BSP_PRV_MEMWAITR_RESETTING_VALUE;

        /* WAIT_LOOP */
        while (BSP_PRV_MEMWAITR_RESET_VALUE != FLASH.MEMWAITR.WORD)
        {
            /* wait for bit to set */
            R_BSP_NOP();
        }
