...
#define BSP_PRV_HOCOCR_RESET_VALUE_00    (0x00)
#define BSP_PRV_HOCOCR_RESET_VALUE_01    (0x01)
#define BSP_PRV_HOCOCR2_RESET_VALUE      (0x00)
...
static void clock_source_select (void)
{
    ...
    if (1 == SYSTEM.HOCOCR.BIT.HCSTP)
    {
        ...
        if(0x00 == SYSTEM.HOCOCR2.BYTE)      // 32MHz
        {
            SYSTEM.HOCOTRR0.BYTE = (0x3F & BSP_CFG_HOCO_TRIMMING_REG_VALUE);
        }
        else                                 // 54MHz
        {
            SYSTEM.HOCOTRR3.BYTE = (0x3F & BSP_CFG_HOCO_TRIMMING_REG_VALUE);
        }
        #endif
...
#if (BSP_CFG_MAIN_CLOCK_OSCILLATE_ENABLE == 1) || (BSP_CFG_USB_CLOCK_SOURCE == 1)
...
#endif /* (BSP_CFG_MAIN_CLOCK_OSCILLATE_ENABLE == 1) || (BSP_CFG_USB_CLOCK_SOURCE == 1) */
...
void bsp_mcu_clock_reset_bootloader (void)
{
...
#if BSP_CFG_HOCO_OSCILLATE_ENABLE == 1
    /* HOCO oscillation is enabled after a reset. */
    if(0 == (BSP_CFG_OFS1_REG_VALUE & 0x00000100))
    {
        /* HOCO Stop? */
        if(BSP_PRV_HOCOCR_RESET_VALUE_00 != SYSTEM.HOCOCR.BYTE)
        {
            /* Is HOCOCR2 reset value? */
            if(BSP_PRV_HOCOCR2_RESET_VALUE != SYSTEM.HOCOCR2.BYTE)
            {
                /* Reset HOCOCR2. */
                SYSTEM.HOCOCR2.BYTE = BSP_PRV_HOCOCR2_RESET_VALUE;
            }

            /* Reset HOCO (operating). */
            SYSTEM.HOCOCR.BYTE = BSP_PRV_HOCOCR_RESET_VALUE_00;
    ...
    {
        /* HOCO Operating? */
        if(BSP_PRV_HOCOCR_RESET_VALUE_01 != SYSTEM.HOCOCR.BYTE)
        {
            /* Reset HOCO (Stop). */
            SYSTEM.HOCOCR.BYTE = BSP_PRV_HOCOCR_RESET_VALUE_01;
