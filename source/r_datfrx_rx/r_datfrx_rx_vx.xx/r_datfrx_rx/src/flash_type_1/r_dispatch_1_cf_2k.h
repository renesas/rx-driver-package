...
*************************************************************************************************/
/************************************************************************************************
* File Name    : r_dispatch_1_cf_2k.h
* Version      : 2.00
...
Macro definitions
*************************************************************************************************/
#ifndef R_DISPATCH_1_CF_2K_H
#define R_DISPATCH_1_CF_2K_H
...
#define FLASH_DM_BH_ERASE_FLAG_SIZE ((uint16_t)(16))
#define FLASH_DM_BH_FLAG_SIZE       ((uint16_t)(16))
...
#define FLASH_DM_DH_FLAG_SIZE       ((uint16_t)(8))
#define FLASH_DM_FLASH_BLOCK_SIZE   ((uint32_t)(2048))
#define FLASH_DM_BLOCK_MAX_MASK     ((uint32_t)(0xfffff800))
...
#define FLASH_DM_PRG_BUF_INDEX      (2)
...
typedef struct
{
    flash_dm_t  erase_start[4]; /* 16 bytes */
    flash_dm_t  erase_end[FLASH_DM_BH_ERASE_FLAG_INDEX]; /* 16 bytes */
    flash_dm_t  ready[4];       /* 16 bytes */
    flash_dm_t  active[4];      /* 16 bytes */
    flash_dm_t  full[4];        /* 16 bytes */
    flash_dm_t  reclaim[4];     /* 16 bytes */
} st_flash_dm_bh_t;                /* 96 bytes */
...
typedef struct
{
    flash_dm_t  header_exist[2];    /*  8 bytes */
    uint8_t     no;                 /*  1 byte  */
    uint8_t     addr_h;             /*  1 byte  : data address[2] -> xxXXxxxxh */
    uint16_t    addr;               /*  2 bytes : data address[1] + data address[0] -> xxxxXXXXh */
    uint32_t    rsv;                /*  4 bytes */
    flash_dm_t  header_valid[2];    /*  8 bytes */
    flash_dm_t  data_valid[2];      /*  8 bytes */
    flash_dm_t  validity[2];        /*  8 bytes */
} st_flash_dm_dh_t;                    /* 40 bytes */
...
typedef struct
{
    ...
    uint8_t             nblock;
    uint8_t             rsv[3];
    /* cast from uint8_t to e_flash_dm_drver_status */
    e_flash_dm_driver_status_t stack[FLASH_DM_STACKSIZE]; /* Execution stack */
} st_flash_dispatch_1_hndl_t;      /* 156(+2) bytes */
