...
*              : 31.05.2023 2.00     Changed return value for parameter error in R_LCDC_AdjustContrast.
*                                    Fixed typo in function name.
*                                    Changed the argument check of the R_LCDC_Control function.
*                                    Changed register reflection timing of lcdc_control function.
*                                    Removed the lcdc_delay function.
*                                    Changed the processing of the lcdc_delay_us function to
*                                    use the R_BSP_SoftwareDelay function.
*                                    Fixed the process that may set LCDM1 while SCOC bit is "1".
*                                    Fixed the problem that the LCD count source could not be stopped when closing.
*                                    Removed the LCDC_SEL_CIRCUIT_ENABLE, LCDC_SEL_CIRCUIT_DISABLE.
*******************************************************************************/
/*******************************************************************************
...
lcdc_err_t R_LCDC_OperateCircuit(uint8_t select_operation)
{
    ...
    if((LCDC_CIRCUIT_ENABLE != select_operation) && (LCDC_CIRCUIT_DISABLE != select_operation))
    {
        return LCDC_ERR_INVALID_ARG;
    }
    ...
    if (LCDC_CIRCUIT_ENABLE == select_operation)
    {
        /* Calls the API function. */
        ret = lcdc_operatecircuit_enable();
    }
    
    else if (LCDC_CIRCUIT_DISABLE == select_operation)
    {
        /* Calls the API function. */
...
static lcdc_err_t lcdc_operatecircuit_enable(void)
{
    ...
    /* Backup and clear LCDM1.SCOC */
    old_scoc = LCDC.LCDM1.BIT.SCOC;
...
lcdc_err_t R_LCDC_Control(uint8_t config_pattern, uint8_t select_drv_gen, uint8_t select_display_area)
{
    ...
    if ((0x00 == (config_pattern & LCDC_GEN_ERR_CON)) || (0x00 != (config_pattern & (~LCDC_GEN_ERR_CON) )))
    {
        return LCDC_ERR_INVALID_ARG;
    }
    ...
    /* Calls the API function. */
    ret = lcdc_control(config_pattern, select_drv_gen, select_display_area);

    return ret;
}

/*---------------------------------------------------------------------------*/
static lcdc_err_t lcdc_control(uint8_t config_pattern, uint8_t select_drv_gen, uint8_t select_display_area)
{

    /* Sets LCDC_SUCCESS as initialization of ret. */
    lcdc_err_t ret = LCDC_SUCCESS;
    
    /* Register setting buffer */
    uint8_t buf_mdset  = LCDC.LCDM0.BIT.MDSET;
    uint8_t buf_lcdc_vol_method = g_lcdc_vol_method_flg;
    uint8_t buf_blon   = LCDC.LCDM1.BIT.BLON;
    uint8_t buf_lcdsel = LCDC.LCDM1.BIT.LCDSEL;
    uint8_t old_scoc;
    ...
    if((config_pattern & LCDC_FIX_DRV_GENERETOR) == LCDC_FIX_DRV_GENERETOR)
    {
        ...
        { 
            buf_mdset = LCDC_VOL_CAPACITOR;
            buf_lcdc_vol_method = LCDC_VOL_METHOD_CAPA;
        }
 
        /* Switch to internal voltage boosting method from capacitor spilit method. */
        else if(LCDC_DRV_CAPA_TO_INTERNAL == select_drv_gen) 
        {
            buf_mdset = LCDC_VOL_INTERNAL;
            buf_lcdc_vol_method = LCDC_VOL_METHOD_INTERNAL;
        }
        else
        {
            ret = LCDC_ERR_OTHER;
    ...
    if((config_pattern & LCDC_FIX_DISPLAY_AREA) == LCDC_FIX_DISPLAY_AREA)
    {
        ...
        {
            buf_blon = LCDC_NOT_BLINKING;
            buf_lcdsel = LCDC_DISP_A;
        }
    ...
    /* Register reflection processing */
    if (LCDC_SUCCESS == ret)
    {
        /* Backup and clear LCDM1.SCOC */
        old_scoc = LCDC.LCDM1.BIT.SCOC;
        ...
        {
            /* Display off */
            R_LCDC_DispOff(LCDC_ONLY_DISPLAY_OFF);
            
            LCDC.LCDM0.BIT.MDSET = buf_mdset;
            g_lcdc_vol_method_flg = buf_lcdc_vol_method;
        ...
        if ((config_pattern & LCDC_FIX_DISPLAY_AREA) == LCDC_FIX_DISPLAY_AREA)
        {   
            LCDC.LCDM1.BIT.BLON = buf_blon;
            LCDC.LCDM1.BIT.LCDSEL = buf_lcdsel;
        }
...
* Function Name: lcdc_delay_us
* Description  : Call the R_BSP_SoftwareDelay function and wait for the specified us time.
* Arguments    : delay_us      : Waiting time(us)
* Return Value : LCDC_SUCCESS  : Success
*******************************************************************************/
lcdc_err_t lcdc_delay_us (uint32_t delay_us)
{
    R_BSP_SoftwareDelay(delay_us, BSP_DELAY_MICROSECS);
