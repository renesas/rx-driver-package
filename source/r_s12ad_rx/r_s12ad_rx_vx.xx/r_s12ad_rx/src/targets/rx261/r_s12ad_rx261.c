                        ...
                        void *    const   p_args)
{
    ...
    case ADC_CMD_SCAN_NOW:
        ...
        {
            PORTB.PODR.BIT.B6 = ~PORTB.PODR.BIT.B6;
...
static adc_err_t adc_en_comparator_level0(adc_cmpwin_t  *p_cmpwin)
{
    ...
    if ((p_cmpwin->compare_mask & 0x0000F800) != 0)
#elif BSP_PACKAGE_PINS == 80
    if ((p_cmpwin->compare_mask & 0xF8C0F800) != 0)
#elif BSP_PACKAGE_PINS == 64
    if ((p_cmpwin->compare_mask & 0xFFC0F800) != 0)
#elif BSP_PACKAGE_PINS == 48
    if ((p_cmpwin->compare_mask & 0xFFE1F818) != 0)
#endif
    {
        return ADC_ERR_INVALID_ARG;
    }
    ...
    if (true == p_cmpwin->windowa_enable) // COMPARATOR A
    {
        ...
        p_regs->ADCMPANSR0.WORD = p_cmpwin->compare_mask & 0x01FF;      // Select channels to compare.  Channels 0 to 8.
        p_regs->ADCMPLR0.WORD = p_cmpwin->inside_window_mask & 0x01FF;  // Select the CoMPare Level. Channels 0 to 8.
...
static adc_err_t adc_configure_scan(adc_ch_cfg_t   *p_config)
{
    ...
    p_regs->ADANSA0.WORD = (uint16_t) (p_config->chan_mask & 0x01FF);            //CH0 to CH8
    p_regs->ADANSB0.WORD = (uint16_t) (p_config->chan_mask_groupb & 0x01FF);     //CH0 to CH8

    /* Select the channels for Addition/Averaging (ch0 to ch7) */
    p_regs->ADADS0.WORD  = (uint16_t) (p_config->add_mask & 0x01FF);             //CH0 to CH8
...
static adc_err_t adc_check_scan_config(adc_ch_cfg_t   *p_config)
{
    ...
    || ((p_config->chan_mask & 0xF8C0F800) != 0)
#elif BSP_PACKAGE_PINS == 64
    || ((p_config->chan_mask & 0xFFC0F800) != 0)
#elif BSP_PACKAGE_PINS == 48
    || ((p_config->chan_mask & 0xFFE1F818) != 0)
#endif
...
#if BSP_PACKAGE_PINS == 80
        || ((p_config->chan_mask_groupb & 0xF8C0F800) != 0)
#elif BSP_PACKAGE_PINS == 64
        || ((p_config->chan_mask_groupb & 0xFFC0F800) != 0)
#elif BSP_PACKAGE_PINS == 48
        || ((p_config->chan_mask_groupb & 0xFFE1F818) != 0)
#endif
