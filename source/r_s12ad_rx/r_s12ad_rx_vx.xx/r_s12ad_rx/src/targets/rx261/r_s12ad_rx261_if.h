...
#ifndef S12AD_PRV_RX261_IF_H
#define S12AD_PRV_RX261_IF_H
...
#define ADC_MASK_TEMP   (1<<9)     /* temperature sensor */
#define ADC_MASK_VOLT   (1<<10)     /* internal reference voltage sensor */
...
typedef enum e_adc_trig              // trigger sources (set to TRSA bit or TRSB bit)
{
    ...
    ADC_TRIG_SYNC_ELCTRG00N              = 9,     // ELC.ELSR15
    ADC_TRIG_SYNC_ELCTRG01N              = 10,    // ELC.ELSR56
    ADC_TRIG_SYNC_ELCTRG00N_OR_ELCTRG01N = 11,    // ELC.ELSR15 or ELC.ELSR56

    ADC_TRIG_SOFTWARE                    = 32,   /* software trigger; not for Group modes
                                                    nor double trigger modes
...
/* --------------------------------------------------------------------------------------------------------------------
   adc_cycle_t         | adc_speed_t             | Formula Calculated                                    | value
-------------------------------------------------|-------------------------------------------------------|-------------
   ADC_CONVERT_3cycles | ADC_CONVERT_SPEED_HIGH  | Number of cycles required to convert one bit * 12 – 4 | 32 cycles
   ADC_CONVERT_3cycles | ADC_CONVERT_CURRENT_LOW | Number of cycles required to convert one bit * 12 + 5 | 41 cycles
   ADC_CONVERT_2cycles | ADC_CONVERT_SPEED_HIGH  | Number of cycles required to convert one bit * 12 – 2 | 22 cycles
   ADC_CONVERT_2cycles | ADC_CONVERT_CURRENT_LOW | Number of cycles required to convert one bit * 12 + 4 | 28 cycles
---------------------------------------------------------------------------------------------------------------------*/
...
typedef enum e_adc_cycle
{
    ADC_CONVERT_3CYCLES   = 0,   // ADCCR.BIT.CCS = 0,
    ADC_CONVERT_2CYCLES   = 1    // ADCCR.BIT.CCS = 1,
} adc_cycle_t;

typedef struct st_adc_cfg
{
    adc_speed_t     conv_speed;      // ADCSR.ADHSC, rx140 has two conversion speeds
    ...
    adc_cycle_t     conv_cycle;      // ADCCR.CCS, Selects the number of cycles required to convert; 2cycles or 3cycles
} adc_cfg_t;
...
typedef enum e_adc_sst_reg          // sample state registers
{
    ...
    ADC_SST_CH7,
    ADC_SST_CH8,
    ADC_SST_CH16_TO_31,                        // Use for ADSSTRL register
    ADC_SST_TEMPERATURE,
    ADC_SST_VOLTAGE,
...
{
    uint32_t        chan_mask;          // channels/bits 0-8, 16-31
