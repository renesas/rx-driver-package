...
#define ADC_PRV_SST_REG_MAX(x)     (((x)==0) ?  ADC_SST_CH3 : \
                                    ((x)==1) ?  ADC_SST_CH3 : \
                                                ADC_SST_REG_MAX)
...
#if ADC_PRV_RAMSIZE_IS_48K
#define ADC_PRV_UNIT_CHECK ((unit > 2) || (unit == 1))
#else
#define ADC_PRV_UNIT_CHECK (unit > 2)
#endif /* ADC_PRV_RAMSIZE_IS_48K */
...
volatile uint16_t R_BSP_EVENACCESS_SFR * const  gp_dreg1_ptrs[1] =
                    {   NULL,                /* Dummy is placed because there is no Unit1 in RAM capacity 48K */
                    };
                    ...
                    void         (* const  p_callback)(void *p_args))
{
    ...
    if (ADC_ADCS_GROUP_SCAN == p_regs->ADCSR.BIT.ADCS)
    {
        ...
        p_regs->ADGCTRGR.BIT.TRSC = p_cfg->trigger_groupc & 0x3F;      // for Group C
        p_regs->ADGCTRGR2.BIT.TRSC6 = p_cfg->trigger_groupc >> 6;
        if (0 == unit)
        {
    ...
    p_regs->ADCER.BIT.ASE = (ADC_AUTO_SET_ON == p_cfg->setting) ? 1 : 0;
    if (2 == unit)
    {
        if (ADC_TEMP_SENSOR_NOT_AD_CONVERTED != p_cfg->temp_sensor)
        {
                                    ...
                                    void         (* const  p_callback)(void *p_args))
{
    ...
    if (ADC_PRV_UNIT_CHECK || (p_cfg->priority > BSP_MCU_IPL_MAX))
    {
                                    ...
                                    adc_ch_cfg_t   *p_config)
{
    ...
    if (ADC_GRPA_PRIORITY_OFF != p_config->priority_groupa)
    {
        ...
        tmp_trig_c = p_regs->ADGCTRGR.BIT.TRSC;
        trig_c     = tmp_trig_c + (p_regs->ADGCTRGR2.BIT.TRSC6 << 6);             // save TRSC
        ...
        if (0 == p_regs->ADGSPCR.BIT.GBRP)
        {
            ...
            p_regs->ADGCTRGR.BIT.TRSC = trig_c & 0x3F;  // restore TRSC
            p_regs->ADGCTRGR2.BIT.TRSC6 = trig_c >> 6;  // restore TRSC
        }
        else
        {
                                        ...
                                        adc_ch_cfg_t   *p_config)
{
    ...
    /* Confirm that the temperature sensor settings of R_
    function and R_ADC_Control function match. */
