...
static adc_err_t adc_en_comparator_level0(adc_cmpwin_t  *p_cmpwin)
{
    ...
    if (true == p_cmpwin->windowa_enable) // COMPARATOR A
    {
        ...
        S12AD.ADCMPANSR1.WORD = (p_cmpwin->compare_mask>>16) & 0x073F;  // Select channels to compare. 16 to 21, 24 to 26.
        S12AD.ADCMPLR1.WORD = (p_cmpwin->inside_window_mask>>16) & 0x073F;
...
static adc_err_t adc_configure_scan(adc_ch_cfg_t   *p_config)
{
    ...
    S12AD.ADANSA1.WORD = (uint16_t) ((p_config->chan_mask >> 16) & 0x073F);      //CH16 to CH21, CH24 to CH26

    /* Select the channels from among Ch16 to Ch31 for conversion */
    S12AD.ADANSB1.WORD = (uint16_t) ((p_config->chan_mask_groupb >> 16) & 0x073F);

    /* Select the channels for Addition/Averaging (Ch16 to Ch31) */
    S12AD.ADADS1.WORD  = (uint16_t) ((p_config->add_mask >> 16) & 0x073F);
