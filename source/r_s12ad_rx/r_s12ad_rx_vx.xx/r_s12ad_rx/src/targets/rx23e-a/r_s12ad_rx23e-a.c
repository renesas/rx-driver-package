...
*
* Arguments    : unit -
*                    Ignored for rx23e-a. Pass 0 for this argument.
*                mode -
...
*       with this restriction include ADANSA0, ADANSB0, 
*       ADADC, ADADS0, ADSSTRx, ADCMPCR, ADCMPANSR0,
*       ADCMPLR0, ADCMPDR0,
...
static adc_err_t adc_en_comparator_level0(adc_cmpwin_t  *p_cmpwin)
{
    ...
    if (true == p_cmpwin->windowa_enable) // COMPARATOR A
    {
        ...
        p_regs->ADCMPANSR0.WORD = p_cmpwin->compare_mask & 0x003F;   // Select channels to compare.  Channels 0 to 5.
        p_regs->ADCMPLR0.WORD = p_cmpwin->inside_window_mask & 0x003F;   // Select the CoMPare Level. Channels 0 to 5.
...
static uint32_t adc_get_and_clr_cmpi_flags(void)
{
    ...
    /* Get flags for channels AN00 to AN05 */
...
static adc_err_t adc_configure_scan(adc_ch_cfg_t   *p_config)
{
    ...
    /* Select the channels from among Ch0 to Ch5 for conversion */
    p_regs->ADANSA0.WORD = (uint16_t) (p_config->chan_mask & 0x003F);            //CH0 to CH5
    p_regs->ADANSB0.WORD = (uint16_t) (p_config->chan_mask_groupb & 0x003F);     //CH0 to CH5

    /* Select the channels for Addition/Averaging (ch0 to ch5) */
    p_regs->ADADS0.WORD  = (uint16_t) (p_config->add_mask & 0x003F);             //CH0 to CH5
...
static adc_err_t adc_check_scan_config(adc_ch_cfg_t   *p_config)
{
    ...
    /* When VREFL0 is to be used, AN002 cannot be used. */
    if ((1 == S12AD.ADHVREFCNT.BIT.LVSEL) && (0 != (p_config->chan_mask & ADC_MASK_CH2)))
    {
        return ADC_ERR_ILLEGAL_ARG;
    }

    /* When VREFH0 is to be used, AN003 cannot be used. */
    if ((1 == S12AD.ADHVREFCNT.BIT.HVSEL) && (0 != (p_config->chan_mask & ADC_MASK_CH3)))
    {
        return ADC_ERR_ILLEGAL_ARG;
    }
    ...
    if (ADC_ADCS_GROUP_SCAN == p_regs->ADCSR.BIT.ADCS)
    {
        ...
        /* When VREFL0 is to be used, AN002 cannot be used. */
        if ((1 == S12AD.ADHVREFCNT.BIT.LVSEL) && (0 != (p_config->chan_mask_groupb & ADC_MASK_CH2)))
        {
            return ADC_ERR_ILLEGAL_ARG;
        }

        /* When VREFH0 is to be used, AN003 cannot be used. */
        if ((1 == S12AD.ADHVREFCNT.BIT.HVSEL) && (0 != (p_config->chan_mask_groupb & ADC_MASK_CH3)))
        {
            return ADC_ERR_ILLEGAL_ARG;
        }
    }
...
* Description  : This is the RXRX23E-A implementation of R_ADC_Close().
*                This function ends any scan in progress, disables interrupts,
*                and removes power to the A/D peripheral.
* Arguments    : unit - This argument is ignored for the RX23E-A.
...
* Description  : This is the RX23E-A implementation of R_ADC_ReadAll().
