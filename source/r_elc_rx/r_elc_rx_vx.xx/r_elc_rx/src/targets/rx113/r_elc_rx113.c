...
void elc_open_init_register (void)
{
    ...
    ELC.ELSR[17].BYTE = 0x00; /* Event Link Setting Register 17 initialize */
    ...
    ICU.IER[0x0A].BIT.IEN0 = 0; /* Interrupt Request Enable Register initialize */
...
static void elc_set_interrupt_operation (elc_link_module_t * const p_elc_module)
{
    switch (p_elc_module->link_module)
    {
        ...
        case ELC_ICU_LPT:
            ICU.IPR[80].BIT.IPR = p_elc_module->link_module_interrupt_level;
            ICU.IR[80].BIT.IR = 0;
